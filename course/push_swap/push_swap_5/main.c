#include "push_swap.h"
int input[100] = {3, 27, 94, 19, 63, 1, 88, 52, 44, 85,
...
void push_to_stack(Stack *stack_a, int *sorted, int *input, int size)
{
    ...
    while (index < size)
    {
        stack_a->arr[index].value = input[index];
        sorted_index = 0;
        while (sorted_index < size)
        {
            if (input[index] == sorted[sorted_index])
            {
                stack_a->arr[index].index = sorted_index;
                break;
            }
            sorted_index++;
        }
...
int main(void)
{
    ...
    push_to_stack(&stack_a, sorted, input, 100);
    int max = 99;
    int chunk = 0;
    while (stack_a.top >= 0)
    {
