...
int swap(Stack *from, Stack *to, int middle)
{
...
void push_swap(Stack *from, Stack *to, int *sorted)
{
    ...
    int index = 0;
    middle = from->top / 2;
    while (from->top > middle)
    {
        index = 0;
        while (index++ < middle)
        {
            if (from->arr[from->top] >= sorted[middle])
                pa(from, to);
            else
                rotate_bottom(from);
        }
...
int main(void)
{
    ...
    push_swap(&stack, &temp, sorted);
    for (int i = 48; i >= 0; i--)
        printf("[%d] - %d - %d \n", i, stack.arr[i], temp.arr[i]);
    /*  push_back(&temp, &stack, sorted);
     for (int i = 99; i >= 0; i--)
         printf("[%d] - %d \n", i, stack.arr[i]); */
    printf("\nTotal operations: %d\n", operation_count);
    printf("\n");
