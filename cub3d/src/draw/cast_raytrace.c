		...
		t_settings **st)
{
	...
	cast->wall_start = (HEIGHT / 2) - ((double)((HEIGHT / cast->wall_distance)
				* 0.5) / 2);
	cast->wall_end = (HEIGHT / 2) + ((double)((HEIGHT / cast->wall_distance)
				* 0.5) / 2);
...
static void	load_texture(t_raycast *cast, t_dda_result ray, t_image_addr addr)
{
	...
	cast->tex_step = (double)addr.height / (double)(cast->wall_end
			- cast->wall_start + 1);
	cast->tex_pos = 0.0;
}
...
void	raytracer_render(t_settings **st)
{
	...
	while (i < WIDTH)
	{
		cast.ray_angle = (*st)->player.pa + (((double)i / (double)WIDTH - 0.5)
				* FFOV);
