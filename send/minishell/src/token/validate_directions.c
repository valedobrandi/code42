...
/*   Updated: 2025/06/23 08:43:09 by bde-albu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	validate_directions(int current_token, t_token_list *token_list, int i)
{
	...
	length = ft_strlen(token_list->tokens[i].value);
	if (i + 1 < token_list->size && is_redirection(token_list->tokens[i
				+ 1].type))
	...
	if (!is_valid_redirection_target(token_list, i))
		return (print_prompt(ERR_TOKEN_NL, 2), 1);
	return (0);
}
